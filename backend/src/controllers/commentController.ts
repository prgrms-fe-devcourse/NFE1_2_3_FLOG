...
export const createComment = async (req: Request, res: Response) => {
  ...
  try {
    const comment = await createCommentService(postId, postType as "Curation" | "Post", authorId, content);
...
export const updateComment = async (req: Request, res: Response) => {
  ...
  try {
    const updatedComment = await updateCommentService(commentId, content, userId);
    if (!updatedComment) {
      res.status(404).json({ message: "수정할 댓글을 찾을 수 없거나 수정 권한이 없습니다." });
      return;
    }
...
export const likeComment = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ message: "좋아요 기능 처리 중 오류가 발생했습니다.", error });
  }
};
...
export const createReplies = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ message: "대댓글 생성 중 오류가 발생했습니다.", error });
  }
};
...
export const getRepliesByCommentId = async (req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ message: "대댓글 조회 중 오류가 발생했습니다.", error });
  }
};
...
export const updateRepliesId = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(200).json({ message: "대댓글이 성공적으로 수정되었습니다.", reply });
  } catch (error) {
    res.status(500).json({ message: "대댓글 수정 중 오류가 발생했습니다.", error });
  }
};
...
export const likeReplies = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(500).json({ message: "대댓글 좋아요 처리 중 오류가 발생했습니다.", error });
  }
};
